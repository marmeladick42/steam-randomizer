...
async function request(url, timeout) {
  try {
    ...
    const reason = err.name === 'TimeoutError' ? 'превышено время ожидания' : err.message;
    throw new SteamError(`Нет связи со Steam: ${reason}`, { code: 'NETWORK' });
  }
}

async function parseResponse(res) {
  if (res.status === 401 || res.status === 403) {
    throw new SteamError('Steam отклонил запрос: ключ API недействителен', { status: res.status, code: 'BAD_KEY' });
  }
  if (res.status === 429) {
    throw new SteamError('Steam ограничил частоту запросов — подождите минуту', { status: 429, code: 'RATE_LIMIT' });
  }
  if (!res.ok) throw new SteamError(`Steam вернул ошибку ${res.status}`, { status: res.status });
  try {
    return await res.json();
  } catch {
    throw new SteamError('Steam вернул некорректный ответ', { status: res.status });
  }
}
...
async function validateKey(key) {
  if (!/^[0-9A-Fa-f]{32}$/.test(key)) {
    throw new SteamError('Ключ должен состоять из 32 символов (0-9, A-F)', { code: 'BAD_FORMAT' });
  }
...
async function resolveSteamId(key, input) {
  ...
  if (!raw) throw new SteamError('Введите ссылку на профиль или SteamID', { code: 'EMPTY' });
  let m = raw.match(/(?:^|\/profiles\/)(7656\d{13})(?:\/|$)/);
  if (m) return m[1];
  m = raw.match(/\/id\/([^/?#]+)/);
  const vanity = m ? m[1] : raw.replace(/^@/, '');
  if (!/^[\w.-]{2,64}$/.test(vanity)) throw new SteamError('Не удалось распознать профиль', { code: 'BAD_PROFILE' });
  const data = await getJson(`${API}/ISteamUser/ResolveVanityURL/v1/?${qs({ key, vanityurl: vanity })}`);
  if (data?.response?.success !== 1) throw new SteamError(`Профиль «${vanity}» не найден`, { code: 'NOT_FOUND' });
...
async function getPlayerSummary(key, steamid) {
  ...
  if (!p) throw new SteamError('Профиль не найден', { code: 'NOT_FOUND' });
...
async function getOwnedGames(key, steamid) {
  ...
  if (!games) {
    throw new SteamError('Библиотека недоступна: сделайте «Игровую информацию» профиля публичной в настройках приватности Steam', {
      code: 'PRIVATE',
    });
  }
...
async function searchPage(params, start) {
  ...
  if (!data || typeof data.results_html !== 'string') throw new SteamError('Поиск Steam вернул неожиданный ответ');
