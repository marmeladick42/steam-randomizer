...
function assertConsistent(f, { library }) {
  let problem = null;
  if (inverted(num(f.minReviews), num(f.maxReviews))) problem = 'Минимум отзывов больше максимума.';
  else if (inverted(num(f.yearFrom), num(f.yearTo))) problem = 'Начальный год выхода позже конечного.';
  else if (!library) {
    const priceMin = num(f.priceMin);
    const priceMax = num(f.priceMax);
    if (inverted(priceMin, priceMax)) problem = 'Минимальная цена больше максимальной.';
    else if (f.priceMode === 'free' && priceMin !== null && priceMin > 0) problem = 'Для бесплатных игр нельзя задать минимальную цену.';
    else if (f.priceMode === 'free' && (f.onSale || num(f.minDiscount))) problem = 'У бесплатных игр не бывает скидок.';
    else if (f.priceMode === 'paid' && priceMax !== null && priceMax <= 0) problem = 'Для платных игр максимальная цена должна быть больше нуля.';
  }
  if (problem) throw new steam.SteamError(`${problem} Исправьте фильтры.`, { code: 'BAD_FILTERS' });
}
...
async function rollStore(f, settings, { exclude, onProgress }) {
  ...
  if (seen.size >= pageCount) {
    throw new steam.SteamError(
      `Проверены все ${total} игр из поиска Steam, но ни одна не прошла дополнительные фильтры (отзывы, цена, год, Steam Deck). Попробуйте их ослабить.`,
      { code: 'NO_MATCH' },
    );
  }
  throw new steam.SteamError(
    `За один заход можно проверить только ${MAX_STORE_PAGES * steam.SEARCH_PAGE_SIZE} игр, иначе Steam ограничит запросы. ` +
      `Проверено ${checked} из ${total}, подходящих среди них не оказалось — они встречаются слишком редко. ` +
      'Попробуйте ещё раз или сузьте поиск тегами.',
    { code: 'SAMPLE_LIMIT' },
  );
}
